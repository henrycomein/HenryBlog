...
namespace Henry.Common
{
    ...
    public class CacheHelper
    {
        ...
        {
            var dependency = new CacheDependency(GetOptionPath());
        ...
        public static void AddWithDependency(string key, object value,string name)
        {
            ...
            HttpRuntime.Cache.Insert(key, value, dependency, Cache.NoAbsoluteExpiration, TimeSpan.FromMinutes(slidingExpiration), CacheItemPriority.Low, null);
        }
        public static void AddWithDependency(string key, object value ,string name, DateTime absoluteExpiration)
        {
            var dependency = new CacheDependency(GetOptionPath());
            var slidingExpiration = ExpiredTime(name);
            HttpRuntime.Cache.Insert(key, value, dependency, absoluteExpiration, TimeSpan.FromMinutes(slidingExpiration), CacheItemPriority.Low, null);
        }
    }
}