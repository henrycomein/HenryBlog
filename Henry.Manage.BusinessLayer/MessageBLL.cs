...
namespace Henry.Manage.BusinessLayer
{
    public class MessageBLL
    {
        ...
        public static List<Message> GetList(Message condition)
        {
            return MessageDAL.GetList(condition).ToEntity<Message>();
        }
        ...
        public static PageParamer<Message> GetListWithPage(Message condition)
        {
            int totalcount=0;
            if (condition.PageSize < 0) condition.PageSize = 10;
            if (condition.PageIndex < 1) condition.PageIndex = 1;
            var result= MessageDAL.GetListWithPage(condition,out totalcount).ToEntity<Message>();;
            return new PageParamer<Message> { Items = result,PageIndex=condition.PageIndex,PageSize=condition.PageSize, TotalCount = totalcount };
        }
        ...
        public static Message GetSingleOrDefault(int keyval)
        {
            var condition = new Message  { M_ID = keyval };
            return  MessageDAL.GetList(condition).ToEntity<Message>().FirstOrDefault();
        }
        ...
        public static bool Add(Message info)
        {
            return MessageDAL.AddOrUpdate(info);
        }
        ...
        {
            return MessageDAL.UpdateStatus(id, 0); 
        }
        ...
        {
            return MessageDAL.UpdateStatus(id, 1);
        }
        ...
        {
            return MessageDAL.UpdateStatus(id, 2);
        }
