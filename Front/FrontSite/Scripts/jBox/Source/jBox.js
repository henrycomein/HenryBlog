...
function jBox(type, options) {
	...
	this.options = {
		...
		autoPadding: true,           //henry add 20150127
        paddingValue:null,          //e.g. '20px 50px','10px 5px 10px 3px',make sure the attribute 'autoPadding' has setted to false
	...
	this.defaultOptions = {
		...
		'Confirm': {
			...
			_onOpen: function() {
			    ...
			    this.submitButton.off('click.jBox-Confirm' + this.id).on('click.jBox-Confirm' + this.id, function() { this.options.confirm ? this.options.confirm() : eval(this.source.data('jBox-Confirm-submit'));if(this.options.autoClose){this.close(); } }.bind(this));
			}
		},
	...
	this._create = function() {
	    ...
	    // Create content henry modify 20150127
		this.content = this.options.autoPadding?jQuery('<div/>', { 'class': 'jBox-content' }).css({ width: this.options.width, height: this.options.height, minWidth: this.options.minWidth, minHeight: this.options.minHeight, maxWidth: this.options.maxWidth, maxHeight: this.options.maxHeight }).appendTo(this.container):jQuery('<div/>', { 'class': 'jBox-content' }).css({ width: this.options.width, height: this.options.height, minWidth: this.options.minWidth, minHeight: this.options.minHeight, maxWidth: this.options.maxWidth, maxHeight: this.options.maxHeight,padding:this.options.paddingValue }).appendTo(this.container);
		...
		if (this.isOpen) {
			...
			if (this.options.fade) {
				this.wrapper.stop().animate({opacity: 0}, {
					...
					complete: function() {
					    this.wrapper.remove(); //this.wrapper.css({display: 'none'}) henry modify 20150127
